...
import FaceCube, CoordCube
from CubieCube import DupedEdge

class DupedFacelet(Exception): pass
...
class Search(object):
	...
	def solution(facelets, maxDepth, timeOut, useSeparator = False):
		...
		while True:
						...
						while True:
							...
							if Search.ax[n] > 5:
								if time.time() - tStart > timeOut:
									raise TimeoutError("Timeout, no solution within given time")
			...
			Search.minDistPhase1[n + 1] = max(
				CoordCube.CoordCube.getPruning(CoordCube.CoordCube.Slice_Flip_Prun, CoordCube.CoordCube.N_SLICE1 * Search.flip[n + 1] + Search.slice[n + 1]),
				CoordCube.CoordCube.getPruning(CoordCube.CoordCube.Slice_Twist_Prun, CoordCube.CoordCube.N_SLICE1 * Search.twist[n + 1]+ Search.slice[n + 1])
			)
			# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	def totalDepth(depthPhase1, maxDepth):
		...
		d1 = CoordCube.CoordCube.getPruning(
			...
			(CoordCube.CoordCube.N_SLICE2 * Search.URFtoDLF[depthPhase1] + Search.FRtoBR[depthPhase1]) * 2 + Search.parity[depthPhase1]
		)
		...
		Search.URtoDF[depthPhase1] = CoordCube.CoordCube.MergeURtoULandUBtoDF[Search.URtoUL[depthPhase1]][Search.UBtoDF[depthPhase1]]

		d2 = CoordCube.CoordCube.getPruning(
			CoordCube.CoordCube.Slice_URtoDF_Parity_Prun,
			(CoordCube.CoordCube.N_SLICE2 * Search.URtoDF[depthPhase1] + Search.FRtoBR[depthPhase1]) * 2 + Search.parity[depthPhase1]
		)
		...
		while True:
			...
			Search.minDistPhase2[n + 1] = max(
				...
				CoordCube.CoordCube.getPruning(
					CoordCube.CoordCube.Slice_URFtoDLF_Parity_Prun,
					(CoordCube.CoordCube.N_SLICE2 * Search.URFtoDLF[n + 1] + Search.FRtoBR[n + 1]) * 2 + Search.parity[n + 1]
				)
			)
			## ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
